...
const Cards = () => {
  ...
  // when we get the data from the data base so we use the useEffect hook 
  useEffect(()=>{
  ...
  return (
    ...
    data.map((e, i) =>{
        return(
    <Link to={`/detail/${e.id}`}>
    <div key={i} className='card text-sm font-medium  shadow-lg p-2 hover:-translate-y-3 cursor-pointer  mt-6 transition-all duration-500'>
      <img className='h-60 md:h-72'  alt='xyz' src={e.image}/>
